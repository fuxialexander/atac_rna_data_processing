...
def get_celltype_list(celltype_annot_path):
    ...
    substr_list = ["Stem Cell", "Astrocyte", "strocyte"]
...
def get_variant_list(variants_path):
    variants_df = pd.read_csv(variants_path, sep=",")
    variant_list = list(set(list(variants_df.rsID.values)))
    return variant_list
...
variant_list = get_variant_list(variants_path)
